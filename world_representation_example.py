...
class Example(Base):
    ...
    def initialize(self):
        ...
        # BarInterior
        wall_geometry, floor_geometry, roof_geometry, door_geometry = BarGeometry(1, 1, 1,
                                                                                  my_obj_reader('objects/interior.obj'))
        ...
        # Jukebox
        wood_geo, neon_geo, metal_geo, red_geo, metalmesh_geo, selectcoin_geo, selectsong_geo, vinyl_geo, songs1_geo, songs2_geo, glass_geo = JukeboxGeometry(
            1, 1, 1, my_obj_reader('objects/jukebox.obj'))
        ...
        self.BEER_LIQUID = Mesh(geometry=self.liquid_geo,
                                material=LIQUID_MATERIAL)  # Use the same liquid geometry as the shelf bottles
    ...
    def update(self):
        ...
        beer_offset = 0.7
        base_position = np.array(self.camera.global_position) + (camera_direction * beer_offset) + np.array(
            [0, -0.7, 0])
        ...
        if self.hasBeer:
            ...
            if self.input.is_mouse_button_pressed(2) and self.BEER_LEFT > 0:
                self.beer_animation_progress = min(1.0,
                                                   self.beer_animation_progress + self.beer_animation_speed * self.delta_time)
                ...
                self.beer_animation_progress = max(0.0,
                                                   self.beer_animation_progress - self.beer_animation_speed * self.delta_time)
        ...
        if self.animating_beer and self.spawned_beers:
            ...
            camera_direction = camera_direction / np.linalg.norm(camera_direction)
            beer_offset = 0.7
            target_position = np.array(self.camera.global_position) + (camera_direction * beer_offset) + np.array(
                [0, -0.7, 0])
            ...
            for beer_data in self.spawned_beers:
                ...
                current_position = beer_data['start_position'] + (
                            target_position - beer_data['start_position']) * eased_progress
    ...
    def _setup_bottle_collision_objects(self):
        ...
        self.bottle_collision_manager.add_collision_object([-10.9, 0, 12], [4.4, 0, 1.5], height=1,
                                                           name="Bottle_Bar_Back")  # Increased width and depth to cover full bar
        self.bottle_collision_manager.add_collision_object([-8.7, 0, 13.3], [1, 0, 1.8], height=1,
                                                           name="Bottle_Bar_Side")
        ...
        self.bottle_collision_manager.add_collision_object([0, 0, -10.5], [4.5, 0, 4.5], height=0.5,
                                                           name="Bottle_Stage_Round")

        # Add stage wireframe structure (metal framework)
        # Left vertical support
        self.bottle_collision_manager.add_collision_object([-4, 0, -10], [0.75, 0, 0.75], height=6.0,
                                                           name="Bottle_Stage_Support_Left")
        # Right vertical support
        self.bottle_collision_manager.add_collision_object([4, 0, -10], [0.75, 0, 0.75], height=6.0,
                                                           name="Bottle_Stage_Support_Right")

        # Add musician on stage collision (same size as barman)
        self.bottle_collision_manager.add_collision_object([0, 0, -10], [0.65, 0, 0.65], height=2.5,
                                                           name="Bottle_Musician")

        # Add jukebox
        self.bottle_collision_manager.add_collision_object([0, 0, 14.5], [1.5, 0, 1.4], height=1.7,
                                                           name="Bottle_Jukebox")

        # Add shelf
        self.bottle_collision_manager.add_collision_object([-11.1, 0, 14.8], [4, 0, 0.9], height=3.2,
                                                           name="Bottle_Shelf")

        # Add barman collision
        self.bottle_collision_manager.add_collision_object([-11, 0, 13], [0.65, 0, 0.65], height=2.0,
                                                           name="Bottle_Barman")

        # Add barstools
        for i in range(4):
            self.bottle_collision_manager.add_collision_object([-12.5 + i, 0, 11], [0.3, 0, 0.3], height=1.0,
                                                               name=f"Bottle_Barstool_{i + 1}")

        # Add TV table
        self.bottle_collision_manager.add_collision_object([14, 0, -14], [2, 0, 1.5], height=1.5,
                                                           name="Bottle_TV_Table")
